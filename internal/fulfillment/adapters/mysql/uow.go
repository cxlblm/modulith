...
func NewUnitOfWork(db *gorm.DB, bus eventbus.Bus) command.UnitOfWork {
	return dbtx.NewUnitOfWork[command.Repositories](db, "fulfillment.UnitOfWork.RunInTx", bus, func(tx *gorm.DB, pending *dbtx.PendingCollector) command.Repositories {
		return command.Repositories{
			Shipments: NewShipmentRepositoryWithTx(tx, pending, bus),
		}
	})
}
